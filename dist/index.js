...
function diff(left, right) {
     ...
     * @private
     */
    function addChange(value, path, change, diff) {
        diff[path] = change;
        ...
        if ((change === DELETE || change === CREATE) && (Array.isArray(value) || isObject(value))) {
            ...
            traverse(value, path, (v, childPath) => diff[childPath] = change);
        }
    }
    /**
     ...
     * @private
     */
    function _calculateDiff(leftObj, rightObj, pathLeft = "", pathRight = "") {
        // iterate over two arrays
        if (Array.isArray(leftObj) && Array.isArray(rightObj)) {
            arrayDiff(leftObj, rightObj, (change, aIndex, bIndex) => {
...
function arrayDiff(a, b, callback) {
    ...
    let aIndex = 0;
    let bIndex = 0;
    function isCommon(indexA, indexB) {
        return isEqual_1.isEqual(a[indexA], b[indexB]);
    }
    ...
    diff_sequences_1.default(a.length, b.length, isCommon, foundSubsequence);
